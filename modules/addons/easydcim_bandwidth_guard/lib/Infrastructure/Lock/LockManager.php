...
final class LockManager
{
    public function acquire(string $key, int $ttlSeconds = 300): bool
    {
        ...
        $existing = Capsule::table('mod_easydcim_bw_guard_meta')->where('meta_key', 'lock:' . $key)->first();
    ...
    {
        Capsule::table('mod_easydcim_bw_guard_meta')->where('meta_key', 'lock:' . $key)->delete();
    }
}
